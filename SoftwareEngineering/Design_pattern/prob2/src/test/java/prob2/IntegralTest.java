...
class IntegralTest {
    ...
    public void encrypt_test() { //간단한 reverse 의 encrpt 테스트
        ...
        try {
            ...
            encryptWriter.setMode(new Reverse());
            encryptWriter.write(test_str);
            encryptWriter.flush();
            encryptWriter.close();
            reader=new FileReader(file);
            reader.read(act);
            assertEquals('A',act[0]);
            assertEquals('B',act[1]);
            assertEquals('C',act[2]);
            assertEquals('D',act[3]);
            assertEquals('E',act[4]);
        } catch (IOException e) {
    ...
    public void Decrypt_test() { //간단한 reverse의 decrpy 테스트
        ...
        try {
            ...
            decryptReader.setMode(new Reverse());
    ...
    public void all_test() { //통합 테스트. 랜덤 문자열에 대해 encrpy-write 하고 다시 Decrypt-read했을 때 원래 값을 되찾는가?
        ...
        try {
            for(int i=0;i<MAX;i++) {
                ...
                EncryptWriter encryptWriter = new EncryptWriter(new FileWriter(file));
                if(i%2==0) {
                    encryptWriter.setMode(new Caesar());
                }
                else{
                    encryptWriter.setMode(new Reverse());
                }
                ...
                decryptReader = new DecryptReader(new FileReader(file));
                if(i%2==0) {
                    decryptReader.setMode(new Caesar());
                }
                else{
                    decryptReader.setMode(new Reverse());
                }
