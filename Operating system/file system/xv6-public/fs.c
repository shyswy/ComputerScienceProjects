...
stati(struct inode *ip, struct stat *st)
{
  ...
  st->nlink = ip->nlink;
  st->size = ip->size;
  st->permission=ip->permission;//project4
  strncpy(st->owner,ip->owner,16);//project4
}
...
dirlookup(struct inode *dp, char *name, uint *poff)
{
  ...
  for(off = 0; off < dp->size; off += sizeof(de)){
    ...
    if(de.inum == 0)
      continue;
    if(namecmp(name, de.name) == 0){//find!
...
int	bit=get_modeBit(ip,MODE_XUSR,MODE_XOTH);//while namex, check execute auth
	if(bit==0){
		iunlockput(ip);
		return 0;
	}
...
int give_userS(struct inode* tmp){
	userS=tmp;
	return 0;
...
int update(struct inode* userS){
	//char* p=user_info.priv;
...
int login(char* username,char* passward){
	for(int i=0;i<10;i++){
		//cprintf("user: %s, psw: %s\n",user_info.priv[i][0],user_info.priv[i][1]);
		if( strncmp(username,user_info.priv[i][0],16)==0 ){
			if(strncmp(passward,user_info.priv[i][1],16)==0) {//id, psw match!
				user_info.using_idx=i;
				return 0;
			}
			cprintf("error: wrong passward\n");
			return -1;
		}
	}
	cprintf("error: no such username exist\n");
	return -1;
...
//	int tmp=-1;
	for(int i=0;i<10;i++){
		if(user_info.priv[i][0][0]=='x'){//find empty space!
			return i;
		}
	}
	return -1;
...
int addUser(char* username,char* passward){//find empty space and add user.
	int idx=find_empty();
	...
	/*
	if(idx==-1){
		cprintf("error: not full, no empyty space, it should not print!\n");
		return -1;
	}
	*/
	...
	for(int i=0;i<10;i++){
		if(strncmp(user_info.priv[i][0],username,16)==0){
			cprintf("error: alreay registered\n");
			return -1;
		}

	}
	strncpy(user_info.priv[idx][0], username, 16);
    strncpy(user_info.priv[idx][1], passward, 16);
	update(userS);
	user_info.num++;
...
//return apropriate bit depends on who(owner or others) is doing what(r, w, exe) 
	if(initial==0){ 
		return 1;
	}

	if(strncmp(ip->owner,get_now(),16)==0||strncmp("root",get_now(),16)==0){//if owner, give mode bit for owner 
...
const char* get_now(){
    ...
    return user_info.priv[user_info.using_idx][0];
}
