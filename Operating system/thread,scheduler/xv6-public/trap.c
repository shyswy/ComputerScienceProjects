...
tvinit(void)
{
  ...
  SETGATE(idt[128], 1, SEG_KCODE<<3, vectors[128], DPL_USER);//practice01
...
trap(struct trapframe *tf)
{
  ...
  if(myproc() && myproc()->state == RUNNING &&
     tf->trapno == T_IRQ0+IRQ_TIMER){
	  ...
	  //ticks= time  interupt come every 1 cycle (round robin) ticks is tick for queue
