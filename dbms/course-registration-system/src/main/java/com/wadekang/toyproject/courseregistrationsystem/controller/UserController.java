...
public class UserController {
    ...
    @GetMapping("/myCourses")   //mycourseList는 아직 성적 기입 x 수업만 표시
    ...
    @GetMapping("/historyList")  //성적기입 완료된 수업 표시. 성적이 B0 이상이면 재수강 불가, B0 이하이면 수강취소 버튼 표시
    public String myHistoryList(@AuthenticationPrincipal User user, Model model,
        ...
        model.addAttribute("EndTakeClasses", takeClassService.findEndClass(user.getUserId()) ); //B0 이상, 성적입력 완료 과목만
...
//  String classId=pathVars.get("classId");
    ...
    public String GradeUpdate(  //html에서 url에 유저 아이디와 takeid를 전달해준다.
        ...
        try {
            ...
            if(takeClassUpdateRequestDto.getGrade()!=99L) //만약 성적이 올바르게 기입되어 default가 아니게되면, 이수완료이므로 현재 curstudent 수 감소.
